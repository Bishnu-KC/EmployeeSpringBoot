...
public class EmployeeRepository {
	...
	public Object addOne(Employee employee)
	{
		return jdbc.update("insert into employee(id,first_name,last_name,empDeptNo,empHireDate,active)"+"values(?,?,?,?,?,?)",
				new Object[] {employee.getId(),employee.getFirst_name(),employee.getLast_name(),employee.getEmpDeptNo(),
						new Timestamp(employee.getEmpHireDate().getTime()),employee.isActiv()});
	}
	...
	public int UpdateOne(Employee employee)
	{
		return jdbc.update(
				...
				new Object[] {employee.getFirst_name(),employee.getLast_name(),employee.getEmpDeptNo(), new Timestamp(employee.getEmpHireDate().getTime()),
				employee.isActiv(),employee.getId()		
	...
	class EmployeeRowMapper implements RowMapper<Employee>
	{
		...
		public Employee mapRow(ResultSet rs, int arg1) throws SQLException {
			...
			employee.setEmpDeptNo(rs.getShort("empDeptNo"));
			employee.setEmpHireDate(rs.getTimestamp("empHireDate"));
			employee.setActiv(rs.getBoolean("active"));
